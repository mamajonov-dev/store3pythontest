...
def userprorfileview(request):
    ...
    baskets = BasketModel.objects.filter(owner=profile, ordered=False)
    baskets_summa = 0
...
def add_basket_view(request, pk):
    try:
        ...
        basket = BasketModel.objects.filter(product=product, owner=owner, ordered=False)
        if not basket:
...
def delete_basket_view(request, pk):
    basket = BasketModel.objects.get(id=pk)
    basket.delete()
    return HttpResponseRedirect(request.META['HTTP_REFERER'])
